...
# true - Earth, false - oscillator - in set_params
Earth = True
Oscillator = False

choose_field = Oscillator  # тут выбирать, что рисовать - гравитационное, или осциллятор
...
global r0
global vr_0
# global r_norm # к оптимизации работы программы - сразу заполнять этот массив по ходу счета
...
def update(val):
    ...
    v_fi = [v_mod[i] * np.sqrt(abs(1 - (np.dot(v[i], r[i]) / np.linalg.norm(v[i]) / np.linalg.norm(r[i])) ** 2)) for i in range(0, len(v_mod))]  # костыль здесь!
