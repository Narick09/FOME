from math import *
import numpy as np
#расходится не сильно, но сразу же
def eqSolut(U, m, xCoordinate_0, speed_0, total_time: int, step_t):
    x = [xCoordinate_0]#x0
    #step = step_t /2
    #v = []
    #v_1/2 - скорость между точками х0 и х1 - в полуцелые моменты времени
    v = [speed_0]
    t = [0.0]
#мб косяк - случай, когда тупо потенциал константа или когда просто в положении равновесия находится - надо отдельно обработать
    ...
    if speed_0 == 0:
        ...
        a0 = -1.0 * (U(x[1]) - U(x[0])) / (x[1] - x[0]) / m  # между 1 и 0
#        tmp_E = E - U(x[-1])
#        v.append(sqrt(2 * tmp_E / m))#v_1/2
        v.append(v[-1] + a0 * step_t / 2)#v_1/2
    t.append(0.0 + step_t / 2)
    #t.append(0.0 + step_t)
#    N = int(total_time / step_t)
    N = int(2 * total_time / step_t)
    a0 = -1.0 * (U(x[1]) - U(x[0])) / (x[1] - x[0]) / m #между 1 и 0
    v.append(v[-1] + a0 * step_t / 2)#v1
    for i in range (2, N):
        if i % 2 == 0:
            x_pr = x[-1]
            #print(x_pr)
            x.append(x_pr + v[-1] * step_t)#x2+
            a0 = -1.0 * (U(x[-1]) - U(x_pr)) / (x[-1] - x_pr) / m   #a1+
        #v.append(v[-1] + a0 * step_t)#v_(n+1/2)
        v.append(speed_0 + a0 * step_t / 2)#v_(n-1/2) & v_n
        t.append(i * step_t / 2)
    t.append(total_time)
